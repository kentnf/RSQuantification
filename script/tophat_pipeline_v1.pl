...
USAGE: tophat_pipeline_v1.pl -i list -s SS -d database -l fr-firststrand [options]
...
# check tophat bin
my $tophat_bin;
if (-s "${FindBin::RealBin}/bin/tophat-1.4.1.Linux_x86_64/tophat" )
{
	 $tophat_bin = "${FindBin::RealBin}/bin/tophat-1.4.1.Linux_x86_64/tophat";
}
else
{
	$tophat_bin = "tophat";
...
foreach my $list (sort keys %list)
{
	...
	if ($sequencing_method eq "PE" || $sequencing_method eq "PS")
	{
		...
		($seq_hash1, $pre_seg_len1) = seq_to_hash($file1);
		($seq_hash2, $pre_seg_len2) = seq_to_hash($file2);
	...
	{
		$file = $list{$list};
		($seq_hash, $pre_seg_len) = seq_to_hash($file);
		%seq_hash = %$seq_hash;
	}
	...
	for(my $i=0; $i<=$cycle; $i++)
	{
		...
		if ($sequencing_method eq "PE" || $sequencing_method eq "PS")
		{
			%select_read = paired_end_filter($output_sam, $output_sam_m, $temp_sam_m, $read_mismatch);
		}
		else
		{
			%select_read = single_end_filter($output_sam, $output_sam_m, $read_mismatch);
		}
...
{
	my ($insam, $outsam, $mismatch) = @_;
	$mismatch =~ s/-N //;
	$mismatch =~ s/--segment-mismatches //;
...
{
	my ($insam, $outsam, $temp, $mismatch) = @_;
	$mismatch =~ s/-N //;
	$mismatch =~ s/--segment-mismatches //;
	# create the mismatch hash
	my %pattern_mismatch = ();
